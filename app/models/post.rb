class Post < ApplicationRecord
    ...
    scope :accessible, -> (id) { joins(:channels).where(channels: {id: User.find(id).belonging_channels.ids}).distinct }  # TODO: 컨트롤러 / 뷰에서 날짜 순으로 바꿔주기
    
    # 이 id의 channel에 속한 answer들 구하기
    # 예: Answer.channel(3) 하면 3번 채널에 속한 answer들 나옴
    scope :channel, -> (id) { joins(:channels).where(channels: {id: id}).distinct }  # TODO: 컨트롤러 / 뷰에서 날짜 순으로 바꿔주기
